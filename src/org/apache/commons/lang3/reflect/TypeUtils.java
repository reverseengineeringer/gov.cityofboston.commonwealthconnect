...
public class TypeUtils
{
  ...
  private static boolean isAssignable(Type paramType, ParameterizedType paramParameterizedType, Map<TypeVariable<?>, Type> paramMap)
  {
    ...
    do
    {
      ...
      localObject2 = (TypeVariable)((Iterator)localObject1).next();
      localType = unrollVariableAssignments((TypeVariable)localObject2, paramParameterizedType);
      localObject2 = unrollVariableAssignments((TypeVariable)localObject2, paramType);
    } while ((localObject2 == null) || (localType.equals(localObject2)) || (((localType instanceof WildcardType)) && (isAssignable((Type)localObject2, localType, paramMap))));
    return false;
  }
    ...
    do
    {
      ...
      if ((paramType instanceof ParameterizedType))
      {
        ...
        for (;;)
        {
          ...
          {
            Type localType = unrollVariables(paramMap, localObject[i]);
            if (localType != null) {
              localObject[i] = localType;
            }
            i += 1;
          }
          paramMap = new HashMap((Map)localObject);
          paramMap.putAll(getTypeArguments(paramType));
        }
        return parameterizeWithOwner(paramType.getOwnerType(), (Class)paramType.getRawType(), (Type[])localObject);
      }
      paramMap = paramType;
    } while (!(paramType instanceof WildcardType));
